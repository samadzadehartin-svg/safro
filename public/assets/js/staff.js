...
function excelTourImportBox(role) {
  ...
  return `<section class="excel-tour-import-box">
    ...
    <div class="row wrap">
      <select id="${select}" class="field" style="max-width:280px">${staffTours()
...
function staffTaskNoteHtml() {
  ...
  return acc?.taskNote
    ? `<div class="staff-task-note"><b>توضیحات مدیریت برای شما:</b><br>${acc.taskNote}</div>`
    : '';
}
function initStaff() {
  mount('staff');
  try {
    ...
    $('app').innerHTML =
      `<div class="card pad login-box"><h2>ورود پنل فروش</h2><p class="small">پنل فروش به‌روزرسانی شد. برای ورود دوباره دکمه زیر را بزن.</p><button class="btn" style="width:100%" onclick="location.reload()">ورود مجدد</button></div>`;
  }
}
function renderLogin() {
  $('app').innerHTML =
    `<div class="card pad login-box"><h2>ورود فروش</h2><p class="small">هر فروش با یوزرنیم و پسورد خودش وارد می‌شود. ورود دمو: فقط رمز staff123</p><input id="user" class="field" placeholder="یوزرنیم" dir="ltr"><input id="pass" class="field" type="password" placeholder="پسورد" dir="ltr"><button class="btn" style="width:100%;margin-top:12px" onclick="doLogin()">ورود</button></div>`;
}
function doLogin() {
  if (loginRole('staff', $('pass').value, $('user').value)) location.reload();
  else alert('یوزرنیم یا پسورد اشتباه است');
}
...
function renderStaffVisaChoice() {
  ...
  const box = $('staffVisaResult');
  if (!box) return;
  if (!v) {
    box.innerHTML = '<div class="empty-state-mini">ویزایی انتخاب نشده است.</div>';
    return;
  }
...
function airlinePickerHtml(targetId) {
  ...
  <div class="airline-picker-list">${
    list
      .map(
        a => `<button type="button" class="airline-picker-card" data-name="${(a.nameFa + ' ' + a.nameEn + ' ' + a.code).toLowerCase()}" onclick="selectAirline('${targetId}','${a.id}')">
    <div class="airline-logo-mini">${airlineLogoHtml(a)}</div>
    <div><b>${a.nameFa}</b><small dir="ltr">${a.nameEn || '—'} ${a.code ? `| ${a.code}` : ''}</small></div>
  </button>`
      )
      .join('') ||
    '<div class="empty-state-mini">مدیر هنوز ایرلاینی را برای فروش فعال نکرده است.</div>'
  }</div>`;
}
function toggleAirlinePicker(targetId) {
  document.querySelectorAll('.airline-picker-panel').forEach(x => {
    if (x.id !== 'airlinePicker_' + targetId) x.classList.remove('on');
  });
  const box = $('airlinePicker_' + targetId);
  if (!box) return;
...
function staffVisaInfoBox() {
  ...
  return `<section id="staffVisaSection" class="card pad staff-section-panel">
    ...
    <div class="staff-visa-grid">${
      list
        .map(
          v => `<div class="staff-visa-card">
      <b>${v.country || '—'} ${v.city ? `- ${v.city}` : ''}</b>
      <span>${Number(v.price || 0) > 0 ? money(v.price) : 'بدون هزینه ویزا'}</span>
      ...
      <p>مدارک: ${v.docs || '—'}</p>
        ...
        .join('') || '<div class="empty-state-mini">هنوز ویزایی در مدیریت فعال نشده است.</div>'
    }</div>
  </section>`;
}
...
function staffHotelCatalogBox() {
  ...
  const groups = active.reduce((acc, h) => {
    ...
    ${
      Object.keys(groups).length
        ? Object.entries(groups)
            .map(
              ([d, rows]) =>
                `<div class="staff-hotel-destination"><h4>${d} <small>${faNum(rows.length)} مورد</small></h4><div class="staff-hotel-grid">${rows
                  .slice(0, 80)
                  .map(
                    h =>
                      `<div class="staff-hotel-card"><b dir="ltr">${h.nameLatin}</b><span>${typeof hotelStars === 'function' ? hotelStars(h.star) : '★'.repeat(Number(h.star) || 3)} ${h.meal ? `<em>${h.meal}</em>` : ''}</span><small>${h.sourceGroup || '—'}${h.catalogType === 'combo' ? ' | پکیج ترکیبی' : ''}</small>${h.dblPrice ? `<small>دو تخته: ${h.dblPrice} | یک تخته: ${h.sglPrice || '—'} | کودک: ${h.childPrice || '—'}</small>` : ''}</div>`
                  )
                  .join('')}</div></div>`
            )
            .join('')
        : '<div class="empty-state-mini">هنوز مدیر هتلی را برای نمایش در پنل فروش فعال نکرده است.</div>'
    }
  </section>`;
}
...
function staffClearCacheHint() {
  alert(
    'بعد از آپلود نسخه جدید در Vercel، روی صفحه Ctrl + F5 بزن. اگر هنوز نسخه قدیمی بود، از DevTools > Application > Service Workers گزینه Unregister را بزن.'
  );
}
...
function renderStaff() {
  ...
  <section id="staffToursSection" class="staff-section-panel">
    ...
    <div class="card table-wrap"><table><thead><tr><th>عکس</th><th>عنوان</th><th>مقصد</th><th>قیمت شروع</th><th>ظرفیت</th><th>وضعیت</th><th>آخرین ویرایش</th><th>عملیات</th></tr></thead><tbody>${list
      .map(
        t => `<tr><td><img src="${t.img}" style="width:55px;height:55px;object-fit:cover;border-radius:12px"></td><td class="staff-tour-title-cell">
  <b>${t.title}</b><br>
  ...
  <div class="staff-inline-actions">
    ...
    <div class="staff-edit-card-list">${list
      .map(
        t => `<div class="staff-edit-card">
  ...
  reader.onload = () => {
    ...
    rows.slice(1).forEach(r => {
      ...
      const price = Number(cellByKey(r, headers, 'price') || 0),
        ...
        showRaw = String(cellByKey(r, headers, 'show_in_buyer') || 'TRUE').toLowerCase();
      const oldPrice = Number(cellByKey(r, headers, 'special_old_price') || 0),
        ...
        endFa = String(cellByKey(r, headers, 'special_end_jalali') || '').trim();
      ...
      if (h) {
        ...
        t.lastEditedBy = user?.name || user?.username || 'فروش';
...
function catalogForStar(star, dest = '') {
  ...
  const base = hotelCatalog().filter(
    h => Number(h.star) === Number(star) && h.enabledForStaff !== false
  );
...
function staffHotelImage(h) {
  return staffHotelPhotos(h)[0] || '../assets/images/hotel-placeholder.svg';
}
...
function selectCatalogHotelForTour(star, id) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (!hid || hid.startsWith('custom-') || !name || /^Hotel\s+\d/.test(name)) {
      row = i;
      break;
    }
  }
  ...
  if (nameBox)
    nameBox.innerHTML = `${h.nameLatin}<small>${h.destination || ''} ${h.meal ? `| ${h.meal}` : ''}</small>`;
  const img = document.querySelector(`#hotelRow_${star}_${row} .hotel-row-thumb`);
  if (img) img.src = staffHotelImage(h);
  if ($(`hotel_${star}_${row}_price`) && !$(`hotel_${star}_${row}_price`).value)
    $(`hotel_${star}_${row}_price`).value =
      Number(String(h.dblPrice || '').replace(/[^\d]/g, '')) || '';
  ...
  if (photosBox)
    photosBox.innerHTML = staffHotelPhotos(h)
      .slice(0, 4)
      .map(x => `<img src="${x}">`)
      .join('');
...
function renderHotelEditor(t) {
  const box = $('hotelEditor');
  if (!box) return;
  box.innerHTML = [3, 4, 5]
    .map(star => {
      const catalog = catalogForStar(star, t?.dest || '');
      const existing = staffSortHotelsByCapacity(
        (t?.hotels || []).filter(h => Number(h.star) === star)
      );
      const rows = [];
      for (let i = 0; i < 8; i++) {
        const c = catalog[i] || {
        ...
        <div>
          ...
          <div class="hotel-row-photos">${photos
            .slice(0, 4)
            .map(x => `<img src="${x}">`)
            .join('')}</div>
        </div>
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (id && name) {
        ...
        out.push({
          ...
          capacity,
          showInBuyer: !!show,
          photos: staffHotelPhotos(cat),
          meal: cat.meal || cat.board || '',
          bookingLink: cat.bookingLink || '',
        });
      }
    }
  });
...
function openForm(id) {
  const t = id ? findTour(id) : null;
  $('modal').classList.add('on');
  setTimeout(() => {
    document.querySelector('.staff-edit-modal-card')?.scrollTo({ top: 0, behavior: 'smooth' });
    $('modal')?.scrollTo({ top: 0, behavior: 'smooth' });
  }, 30);
  $('formTitle').textContent = t ? 'ویرایش تور' : 'افزودن تور';
  $('tid').value = t?.id || '';
  [
    'title',
    'dest',
  ...
  if (t) {
    ...
    $('returnAirline').value = t.returnAirline || t.airline || '';
    $('flightTime').value = t.flightTime || '';
    $('returnFlightTime').value = t.returnFlightTime || t.landingTime || '';
    $('price').value = t.price;
    $('dates').value = (t.dates || []).join('\n');
    $('desc').value = t.desc || '';
    $('itinerary').value = (t.itinerary || []).join('\n');
    $('docs').value = (t.docs || []).join('\n');
    $('includes').value = (t.includes || []).join('\n');
    $('excludes').value = (t.excludes || []).join('\n');
    $('cancel').value = t.cancellation || '';
    $('child').value = t.childPolicy || '';
    $('type').value = t.type;
    $('level').value = t.level || '';
    $('status').value = t.status;
    $('lastMinute').checked = !!t.lastMinute;
    $('oldPrice').value = t.oldPrice || '';
    $('newPrice').value = t.newPrice || '';
    $('dealPercent').value = t.dealPercent || 0;
    $('dealEndsAtFa').value = t.dealEndsAtFa || '';
    renderHotelEditor(t);
    setSectionToggles(t);
  } else {
    ...
    $('oldPrice').value = '';
    $('newPrice').value = '';
    $('dealPercent').value = 10;
    $('dealEndsAtFa').value = '۱۴۰۵/۰۵/۳۰ ۲۳:۵۹';
    renderHotelEditor(null);
    setSectionToggles(null);
  }
}
function closeForm() {
...
function saveTour(e) {
  ...
  try {
    ...
    if (!title || !dest || !duration || !airline || !price)
      return alert('لطفاً عنوان، مقصد، مدت، ایرلاین و قیمت را کامل وارد کنید');
    const ts = tours(),
      id = $('tid')?.value ? Number($('tid').value) : Date.now(),
      type = $('type')?.value || 'international',
      level = $('level')?.value || '',
      cats = [type],
      user = currentStaffUser();
    if (level) cats.push(level);
    const oldTour = findTour(id),
      special = !!$('lastMinute')?.checked,
      parsedEnd = parseFaDiscountDate($('dealEndsAtFa')?.value || '');
...
function delTour(id) {
  if (confirm('تور حذف شود؟')) {
    saveTours(staffTours().filter(t => t.id !== id));
    renderStaff();
  }
}
