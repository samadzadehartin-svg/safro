...
function destinationGuideDetail(t){
  const g=destinationGuide(t.dest,t.title)||t.destinationGuide;if(!g)return '';
...
function filterHome(){
 try{
   ...
   $('tourGrid').innerHTML=list.map(tourCard).join('')||'<div class="card pad" style="grid-column:1/-1">توری پیدا نشد. از پنل مدیریت/کارشناس تور فعال اضافه کنید.</div>';
   renderCompareDock();
 }catch(e){
