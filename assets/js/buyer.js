...
function route(v,id){view=v;if(v==='detail')selectedTour=id;if(v==='booking'){booking.tourId=id;booking.step=1;const t=findTour(id);const entries=visibleHotelEntries(t||{});booking.hotel=entries.some(e=>e.i===selectedHotel)?selectedHotel:(entries[0]?.i||0);booking.date=t?.dates?.[0]||null}renderBuyer()}
...
function renderDetail(t){
  ...
  const flightHtml = sectionOn(t,'flightInfo') ? `<div class="info-grid">${info('مقصد',t.dest)}${info('مدت',t.duration)}${info('ایرلاین رفت',t.airline)}${info('ایرلاین برگشت',t.returnAirline||t.airline)}${info('شماره پرواز',t.flightNumber)}${info('زمان پرواز رفت',t.flightTime)}${info('زمان پرواز برگشت',t.returnFlightTime||t.landingTime)}</div>` : '';
