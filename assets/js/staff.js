...
function importPriceSheet(inputId,resultId){
  ...
  reader.onload=()=>{
    ...
    rows.slice(1).forEach(r=>{
      ...
      if(h){if(price)h.price=price;if(cap>=0)h.capacity=cap;h.showInBuyer=!(showRaw==='false'||showRaw==='0'||showRaw==='no'||showRaw==='غیرفعال');t.lastEditedBy=user?.name||user?.username||'کارمند';t.lastEditedAt=new Date().toISOString();updated++}else notFound++;
    });
...
function openForm(id){
  ...
  ['title','dest','duration','airline','returnAirline','flightTime','returnFlightTime','price','img','dates','gallery','desc','itinerary','docs','includes','excludes','cancel','child','oldPrice','newPrice','dealPercent','dealEndsAtFa'].forEach(x=>{if($(x))$(x).value=''});
  if(t){
...
function saveTour(e){
  ...
  const data={id,title:$('title').value,dest:$('dest').value,duration:$('duration').value,airline:$('airline').value,returnAirline:$('returnAirline').value||$('airline').value,flightTime:$('flightTime').value||'۰۸:۳۰',returnFlightTime:$('returnFlightTime').value||'',price,label:special?'ویژه':'',type,level,categories:cats,rating:4.5,status:$('status').value,lastMinute:special,oldPrice:Number($('oldPrice').value)||0,newPrice:Number($('newPrice').value)||0,dealPercent:special?(Number($('dealPercent').value)||0):0,dealEndsAtFa:$('dealEndsAtFa').value||'',dealEndsAt:parseFaDiscountDate($('dealEndsAtFa').value)?parseFaDiscountDate($('dealEndsAtFa').value).toISOString():new Date(Date.now()+24*3600000).toISOString(),img:(findTour(id)?.img)||DEFAULT_IMG,gallery:($('gallery').value||'').split('\n').map(x=>x.trim()).filter(Boolean),dates:$('dates').value.split('\n').map(x=>x.trim()).filter(Boolean),hotels:collectHotels(price),desc:$('desc').value||'تور ثبت شده توسط کارمند.',includes:($('includes').value||'ترانسفر\nبیمه\nصبحانه').split('\n').filter(Boolean),excludes:($('excludes').value||'').split('\n').filter(Boolean),itinerary:($('itinerary').value||'').split('\n').filter(Boolean),docs:($('docs').value||'').split('\n').filter(Boolean),cancellation:$('cancel').value,childPolicy:$('child').value,sectionVisibility:collectSectionVisibility(),reviews:[],lastEditedBy:user?.name||user?.username||'کارمند',lastEditedAt:new Date().toISOString()};
