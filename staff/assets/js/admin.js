function adminHotelStarsFallback(n) {
  return typeof hotelStars === 'function' ? hotelStars(n) : '★'.repeat(Number(n) || 0);
}

function excelCell(row, idx) {
  return row && row[idx] !== undefined && row[idx] !== null ? row[idx] : '';
}
function excelNum(v) {
  const n = Number(String(v ?? '').replace(/[^\d.-]/g, ''));
  return isNaN(n) ? 0 : n;
}
function excelStar(v) {
  const m = String(v || '').match(/\d/);
  return m ? Number(m[0]) : 3;
}
function normalizeTourMatchName(s) {
  return String(s || '')
    .toLowerCase()
    .replace(/\s+/g, ' ')
    .trim();
...
function findTourForSheet(sheetName) {
  ...
  return (
    tours().find(t => normalizeTourMatchName(t.title) === n) ||
    tours().find(
      t =>
        normalizeTourMatchName(t.title).includes(n) || n.includes(normalizeTourMatchName(t.title))
    ) ||
    null
  );
}
function importExcelTourSheet(inputId, resultId, targetSelectId) {
  ...
  if (typeof XLSX === 'undefined') {
    alert('کتابخانه خواندن اکسل لود نشده است. اینترنت یا CDN را چک کنید.');
    return;
  }
  ...
  reader.onload = e => {
    ...
    wb.SheetNames.forEach(name => {
      ...
      if (
        (!tour || name.toUpperCase() === 'TOUR' || name === 'راهنما') &&
        targetSelectId &&
        $(targetSelectId)
      ) {
      ...
      if (!tour || name === 'راهنما') {
        messages.push(`${name}: تور متناظر پیدا نشد`);
        return;
      }
...
function excelTourImportBox(role) {
  ...
  return `${supabasePanel()}<section class="excel-tour-import-box">
    ...
    <div class="row wrap">
      <select id="${select}" class="field" style="max-width:280px">${tours()
...
function adminBatchUpdatePrices() {
  const ids = [...document.querySelectorAll('.admin-batch-tour-check:checked')].map(o =>
    Number(o.value)
  );
  const amount = Number($('adminBatchAmount').value) || 0;
  const stars = [3, 4, 5].filter(s => $('adminBatchStar' + s)?.checked);
  if (!ids.length || !amount || !stars.length) {
    alert('تور، ستاره و مبلغ را انتخاب کنید');
    return;
  }
  const ts = tours().map(t =>
    ids.includes(t.id)
      ? {
          ...
          hotels: (t.hotels || []).map(h =>
            stars.includes(Number(h.star)) ? { ...h, price: Number(h.price || 0) + amount } : h
          ),
          lastEditedBy: 'مدیریت',
          lastEditedAt: new Date().toISOString(),
        }
  ...
  if (result)
    result.innerHTML = `<div>قیمت هتل‌های انتخاب‌شده برای ${faNum(ids.length)} تور آپدیت شد.</div>`;
...
function sourceLabelFa(key) {
  return key === 'parto' ? 'پرتو' : key === 'alefba' ? 'الفبای سفر' : 'سفرو ایرانیان';
}
function sourceUrl(key) {
  if (key === 'parto') return 'Parto';
  return key === 'alefba' ? 'https://www.alefbatour.com/tour' : 'https://safaroiranian.com/tour/';
}
  ...
  const sources = [
    {
      key: 'safaroIranian',
      title: 'سفرو ایرانیان',
      desc: 'بانک تورهای سفرو ایرانیان؛ مقصدها، هتل، سرویس و قیمت پایه',
      count: stats.safaro,
    },
    {
      key: 'alefba',
      title: 'الفبای سفر',
      desc: 'بانک تورهای الفبای سفر؛ مقصدها، هتل، سرویس و قیمت پایه',
      count: stats.alefba,
    },
    {
      key: 'parto',
      title: 'پرتو',
      desc: 'بانک تورهای پرتو؛ پیش‌فرض فقط مدیریت، قابل انتشار برای فروش و خریدار',
      count: stats.parto || 0,
    },
  ];
  return `<div class="source-import-grid">${sources
    .map(src => {
      const cfg = s[src.key] || {};
      ...
      <div class="source-import-status">${cfg.lastResult || 'هنوز آپدیت روزانه اجرا نشده است.'}</div>${
        src.key === 'parto'
          ? `<div class="parto-publish-box"><div>${(() => {
              const p = partoPublishStats();
              return `کل: ${faNum(p.total)} | منتشرشده: ${faNum(p.published)} | فقط مدیریت: ${faNum(p.adminOnly)}`;
            })()}</div><button class="soft" onclick="adminSetPartoPublish('admin')">فقط مدیریت</button><button class="btn" onclick="adminSetPartoPublish('publish')">انتشار برای فروش و خریدار</button></div>`
          : ''
      }
      <div class="actions">
...
function adminToggleDailySource(key, on) {
  ...
  showToast(
    on
      ? `آپدیت روزانه ${sourceLabelFa(key)} فعال شد`
      : `آپدیت روزانه ${sourceLabelFa(key)} غیرفعال شد`
  );
}
...
function adminSetPartoPublish(mode) {
  setPartoPublishMode(mode === 'publish' ? 'publish' : 'admin');
  renderAdmin();
}
...
function sourceHotelKindFa(kind) {
  return kind === 'tour' ? 'هتل داخل تور' : 'بانک هتل';
}
function sourceHotelName(e) {
  return e.kind === 'tour'
    ? e.hotel.name || e.hotel.nameLatin || 'هتل'
    : e.hotel.nameLatin || e.hotel.name || 'هتل';
}
function sourceHotelDest(e) {
  return e.kind === 'tour'
    ? e.tour.dest || e.tour.destination || 'عمومی'
    : e.hotel.destination || e.hotel.dest || 'عمومی';
}
function sourceHotelSource(e) {
  return e.kind === 'tour'
    ? e.tour.sourceName || e.tour.label || e.tour.lastEditedBy || 'تور واردشده'
    : e.hotel.sourceGroup || e.hotel.source || 'بانک هتل';
}
function sourceHotelEntries() {
  const catalog = hotelCatalog()
    .filter(
      h =>
        h.imported ||
        String(h.sourceGroup || h.source || '').match(
          /الفبای سفر|سفرو|پرتو|PDF|Parto|Safaro|Alefba/i
        ) ||
        String(h.id || '').match(/alefba|safaro|parto/i)
    )
    .map(h => ({ kind: 'catalog', id: h.id, hotel: h }));
  const tourEntries = [];
  tours()
    .filter(
      t =>
        t.sourceImported ||
        t.sourceName ||
        String(t.label || t.lastEditedBy || '').match(/وارد|الفبای سفر|سفرو ایرانیان|پرتو/i)
    )
    .forEach(t => {
      (t.hotels || []).forEach((h, i) =>
        tourEntries.push({ kind: 'tour', id: String(t.id), index: i, tour: t, hotel: h })
      );
    });
...
function sourceHotelFilterOptions() {
  ...
  const sources = [...new Set(entries.map(sourceHotelSource).filter(Boolean))].sort((a, b) =>
    String(a).localeCompare(String(b), 'fa')
  );
  const dests = [...new Set(entries.map(sourceHotelDest).filter(Boolean))].sort((a, b) =>
    String(a).localeCompare(String(b), 'fa')
  );
...
function sourceHotelCurrentEntries() {
  ...
  if (show === 'on')
    entries = entries.filter(e =>
      e.kind === 'catalog' ? e.hotel.enabledForStaff !== false : e.hotel.showInBuyer !== false
    );
  if (show === 'off')
    entries = entries.filter(e =>
      e.kind === 'catalog' ? e.hotel.enabledForStaff === false : e.hotel.showInBuyer === false
  ...
  entries.sort(
    (a, b) =>
      String(sourceHotelDest(a)).localeCompare(sourceHotelDest(b), 'fa') ||
      String(sourceHotelSource(a)).localeCompare(sourceHotelSource(b), 'fa') ||
      String(sourceHotelName(a)).localeCompare(sourceHotelName(b), 'en')
  );
  return entries;
}
...
function sourceHotelCardHtml(e) {
  ...
  const name = sourceHotelName(e),
    dest = sourceHotelDest(e),
    source = sourceHotelSource(e);
...
function renderSourceHotelEditor() {
  const box = $('sourceHotelEditorTable');
  if (!box) return;
      ...
      hotelCatalog().map(h =>
        h.id === id
          ? {
              ...
              price: v.price,
              capacity: v.capacity,
              enabledForStaff: v.show,
              sourceGroup: v.source || h.sourceGroup,
              note: v.note,
              imported: true,
            }
          : h
    ...
    saveTours(
      tours().map(t => {
        ...
        hs[index] = {
          ...
          meal: v.meal,
          board: v.meal,
          price: Number(String(v.price).replace(/[^0-9]/g, '')) || Number(old.price || 0),
          priceText: v.price,
          capacity: v.capacity,
          showInBuyer: v.show,
          note: v.note,
        ...
        return {
          ...
          dest: v.dest,
          hotels: hs,
          lastEditedBy: 'مدیریت - ویرایش هتل منبع',
          lastEditedAt: new Date().toISOString(),
        };
      })
    );
...
function toggleSourceHotelVisibility(kind, id, index) {
  if (kind === 'catalog') {
    saveHotelCatalog(
      hotelCatalog().map(h =>
        h.id === id ? { ...h, enabledForStaff: !(h.enabledForStaff !== false) } : h
      )
    );
  } else {
    const tourId = Number(id);
...
function bulkSourceHotelVisibility(on) {
  ...
  entries
    ...
    .forEach(e => {
      (tourMap[e.id] || (tourMap[e.id] = new Set())).add(e.index);
    });
  if (catalogIds.size)
    saveHotelCatalog(
      hotelCatalog().map(h => (catalogIds.has(h.id) ? { ...h, enabledForStaff: !!on } : h))
    );
  if (Object.keys(tourMap).length)
    saveTours(
      tours().map(t => {
        const set = tourMap[String(t.id)];
        if (!set) return t;
        return {
          ...t,
          hotels: (t.hotels || []).map((h, i) => (set.has(i) ? { ...h, showInBuyer: !!on } : h)),
        };
      })
    );
  renderSourceHotelEditor();
...
function adminCurrencyOptions(selected) {
  ...
  return [...map.values()]
    .map(
      c =>
        `<option value="${c.code}" ${String(c.code).toUpperCase() === selectedCode ? 'selected' : ''}>${c.code} - ${c.nameFa || currencyShortName(c.code)}</option>`
    )
    .join('');
}
function adminTourDomId(id) {
  return String(id).replace(/[^a-zA-Z0-9_-]/g, '_');
}
function adminJsArg(v) {
  return JSON.stringify(String(v));
}
function tourSourceFa(t) {
  return t.sourceName === 'Parto'
    ? 'پرتو'
    : t.sourceName === 'AlefbaSafar'
      ? 'الفبای سفر'
      : t.sourceName === 'SafaroIranian'
        ? 'سفرو ایرانیان'
        : t.sourceName || t.label || 'دستی';
}
  ...
  return `<section id="admin-tour-bank-pricing" class="card pad tour-bank-pricing-section" style="margin-bottom:16px">
    ...
    <div class="tour-bank-toolbar">
      ...
      <select id="tourBankSourceFilter" class="field" onchange="renderTourBankPricing()"><option value="all">همه منابع</option>${[
        ...new Set(tours().map(t => tourSourceFa(t))),
      ]
        .filter(Boolean)
        .sort((a, b) => a.localeCompare(b, 'fa'))
        .map(s => `<option value="${s}">${s}</option>`)
        .join('')}</select>
      <select id="tourBankDestFilter" class="field" onchange="renderTourBankPricing()"><option value="all">همه مقصدها</option>${[
        ...new Set(
          tours()
            .map(t => t.dest)
            .filter(Boolean)
...
function tourBankCurrentTours() {
  ...
  if (q)
    list = list.filter(t =>
      String([t.title, t.dest, t.sourceName, t.sourceKey, t.label].join(' '))
        .toLowerCase()
        .includes(q)
    );
...
function renderTourBankPricing() {
  ...
  if (box) box.innerHTML = tourBankPricingRowsHtml(tourBankCurrentTours());
}
function saveTourBasePriceCurrency(id) {
  const tid = adminTourDomId(id),
    amount = priceNumber($(`tb_base_${tid}`)?.value),
    code = $(`tb_base_cur_${tid}`)?.value || 'IRR';
  saveTours(
    tours().map(t =>
      String(t.id) === String(id)
        ? {
            ...
            price: amount,
            priceCurrency: code,
            prices: normalizeTourPrices(
              [...(t.prices || []), { code, amount, label: 'قیمت پایه' }],
              amount,
              code
            ),
            lastEditedBy: 'مدیریت - بانک تورها',
            lastEditedAt: new Date().toISOString(),
          }
        : t
    )
  ...
  showToast('قیمت پایه تور ذخیره شد');
}
function addTourCurrencyPrice(id) {
  const tid = adminTourDomId(id),
    amount = priceNumber($(`tb_cur_amount_${tid}`)?.value),
    code = ($(`tb_cur_code_${tid}`)?.value || 'USD').toUpperCase(),
    label = $(`tb_cur_label_${tid}`)?.value?.trim() || currencyShortName(code);
  if (!amount) return alert('مبلغ ارز را وارد کن');
  saveTours(
    tours().map(t => {
      if (String(t.id) !== String(id)) return t;
      const others = (t.prices || []).filter(
        p => String(p.code || p.currency).toUpperCase() !== code
      );
      return {
        ...t,
        prices: normalizeTourPrices(
          [...others, { code, amount, label }],
          t.price,
          t.priceCurrency || 'IRR'
        ),
        lastEditedBy: 'مدیریت - قیمت ارزی',
        lastEditedAt: new Date().toISOString(),
      };
    })
  );
  renderTourBankPricing();
  showToast('قیمت ارزی تور ذخیره شد');
}
function removeTourCurrencyPrice(id, code) {
  saveTours(
    tours().map(t =>
      String(t.id) === String(id)
        ? {
            ...t,
            prices: (t.prices || []).filter(
              p => String(p.code || p.currency).toUpperCase() !== String(code).toUpperCase()
            ),
          }
        : t
    )
  ...
  renderTourBankPricing();
  showToast('قیمت ارزی حذف شد');
}
function saveTourHotelRoomPrices(id, index) {
  const tid = adminTourDomId(id);
  const dbl = priceNumber($(`tb_h_dbl_${tid}_${index}`)?.value),
    sgl = priceNumber($(`tb_h_sgl_${tid}_${index}`)?.value);
  const dc = $(`tb_h_dbl_cur_${tid}_${index}`)?.value || 'IRR',
    sc = $(`tb_h_sgl_cur_${tid}_${index}`)?.value || dc;
...
function safeSafaroIranianImportedTours() {
  try {
    return safaroIranianImportedTours();
  } catch (e) {
    console.warn('safaro import preview failed', e);
    return [];
  }
}
...
function adminCleanTestToursHotels() {
  ...
  try {
    saveHotelCatalog(hotelCatalog().filter(h => !isTestHotelItem(h)));
  } catch (e) {}
  try {
    saveTours(
      tours()
        .filter(t => !isTestTourItem(t))
        .map(t => ({ ...t, hotels: (t.hotels || []).filter(h => !isTestHotelItem(h)) }))
    );
  } catch (e) {}
...
function renderAdmin() {
  const os = orders(),
    sales = os.reduce((s, o) => s + Number(o.totalPrice || 0), 0);
  $('app').innerHTML =
    `<div class="card pad row wrap" style="margin-top:22px"><div><span class="badge international">پنل مدیریت</span><h1>داشبورد مدیریت</h1></div><button class="soft" onclick="logoutRole('admin')">خروج</button></div>
  ...
  <section id="admin-hotels" class="card pad hotel-bank-section" style="margin-bottom:16px">
    <div class="hotel-bank-shell">
      ...
      <div class="current-hotels-box hotel-current-tour-box"><h3>لیست تک‌تک هتل‌های ثبت‌شده در تورها</h3><p class="small">اینجا هتل‌هایی که در خود تورها قیمت‌گذاری شده‌اند دیده می‌شوند و می‌توانی نمایش آن‌ها در پنل مشتری را فعال/غیرفعال کنی.</p><div class="row wrap" style="margin-bottom:10px"><button class="soft" onclick="renderCurrentTourHotels()">بروزرسانی لیست</button><button class="btn" onclick="syncCatalogHotelsToTours()">اعمال هتل‌های فعال مدیریت روی تورهای هم‌مقصد</button><select id="addHotelTourId" class="field" style="max-width:260px">${tours()
        .map(t => `<option value="${t.id}">${t.title}</option>`)
        .join(
          ''
        )}</select><button class="soft" onclick="addCurrentTourHotel(Number($('addHotelTourId').value))">افزودن هتل به تور</button></div><div id="currentTourHotelsTable" class="table-wrap"></div></div>
    </div>
  </section>
  ...
  <section id="admin-hotel-booking-photos" class="card pad" style="margin-bottom:16px">
    ...
    <div class="admin-url-help">برای هر هتل می‌توانی یا عکس را انتخاب کنی، یا چند URL عکس وارد کنی. لینک Booking هم جدا ذخیره می‌شود.</div><div class="row wrap booking-admin-controls">
      <select id="bookingPhotoTour" class="field" onchange="renderBookingPhotoHotels()" style="max-width:320px">${tours()
        .map(t => `<option value="${t.id}">${t.title}</option>`)
        .join('')}</select>
...
function bookingPhotoEditorHtml(tourId, index, h) {
  ...
  if (!photos.length)
    return '<div class="empty-state-mini">هنوز عکسی برای ادیت ثبت نشده است.</div>';
  return `<div class="booking-edit-photo-list">${photos
    .slice(0, 18)
    .map(
      (src, pIndex) => `
...
function renderBookingPhotoHotels() {
  ...
  const box = $('bookingPhotoHotels');
  if (!box) return;
  const t = findTour(tourId);
  if (!t) {
    box.innerHTML = '<div class="empty-state-mini">تور انتخاب نشده است.</div>';
    return;
  }
  const list = t.hotels || [];
  ...
  <div class="booking-photo-list">${
    list
      .map(
        (h, i) => `<div class="booking-photo-card">
    ...
    ${bookingPhotoEditorHtml(tourId, i, h)}
      ...
      .join('') || '<div class="empty-state-mini">برای این تور هنوز هتلی ثبت نشده است.</div>'
  }</div>`;
}
function currentBookingPhotos(tourId, index) {
  return (
    findTour(tourId)?.hotels?.[index]?.photos ||
    findTour(tourId)?.hotels?.[index]?.images ||
    []
  ).filter(Boolean);
}
function collectEditedBookingPhotos(tourId, index) {
  return currentBookingPhotos(tourId, index)
    .map((src, pIndex) => {
  ...
  return saveTours(
    tours().map(t => {
      ...
      hs[index] = {
        ...h,
        ...extra,
        photos: [...new Set((photos || []).filter(Boolean))].slice(0, 18),
      };
...
function saveBookingHotelPhotos(tourId, index) {
  ...
  const singleUrls = [0, 1, 2, 3, 4, 5, 6, 7]
    .map(n => $(`bookingPhotoUrl_${tourId}_${index}_${n}`)?.value?.trim())
    .filter(Boolean);
  const bulkUrls = ($(`bookingPhotos_${tourId}_${index}`)?.value || '')
    .split('\n')
    .map(x => x.trim())
    .filter(Boolean);
  const urls = [...new Set([...edited, ...singleUrls, ...bulkUrls])].slice(0, 18);
  const ok = updateBookingHotelPhotos(tourId, index, urls, {
    bookingLink: $(`bookingLink_${tourId}_${index}`)?.value?.trim() || '',
  });
  if (!ok) return;
  renderBookingPhotoHotels();
...
function uploadBookingHotelPhotos(tourId, index, files) {
  ...
  Promise.all(arr.map(file => resizeImageFile(file, 900, 0.72)))
    .then(urls => {
...
function replaceBookingHotelPhotoFile(tourId, index, photoIndex, files) {
  const file = [...(files || [])][0];
  if (!file) return;
  resizeImageFile(file, 900, 0.72).then(url => {
...
function moveBookingHotelPhoto(tourId, index, photoIndex, dir) {
  ...
  const tmp = list[photoIndex];
  list[photoIndex] = list[ni];
  list[ni] = tmp;
  const ok = updateBookingHotelPhotos(tourId, index, list);
  if (!ok) return;
  renderBookingPhotoHotels();
...
function renderContactStaff() {
  ...
  box.innerHTML =
    list
      .map(
        x =>
          `<span class="customer-trail-chip">${x}<button class="chip-x" onclick="removeContactStaff('${x}')">×</button></span>`
      )
      .join('') || '<span class="small">هنوز مسئولی ثبت نشده است.</span>';
}
...
function renderLeads() {
  ...
  box.innerHTML = `<table><thead><tr><th>مشتری</th><th>درخواست</th><th>منبع</th><th>زمان/گشت</th><th>تورهای دیده‌شده</th><th>مسئول فروش</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>${
    list
      .map(
        l => `<tr>
    ...
    <td><button class="danger" onclick="deleteLead('${l.id}')">حذف</button></td>
      ...
      .join('') || '<tr><td colspan="8">هنوز شماره‌ای ثبت نشده است.</td></tr>'
  }</tbody></table>`;
  renderCustomerTrailAdmin();
}
...
function renderCustomerTrailAdmin() {
  const box = $('customerTrailAdminTable');
  if (!box) return;
  ...
  const orderRows = orders().map(o => ({
    ...
    trail: o.viewedTours || [],
    trailText: o.viewedToursText || '',
  }));
  const rows = [...leadRows, ...orderRows].sort((a, b) =>
    String(b.date || '').localeCompare(String(a.date || ''))
  );
  box.innerHTML = `<table><thead><tr><th>نوع</th><th>مشتری</th><th>تاریخ</th><th>تورهای دیده‌شده مشتری</th></tr></thead><tbody>${
    rows
      .map(
        r => `<tr>
    ...
    <td class="admin-trail-cell">${(r.trail || []).map(x => `<span class="customer-trail-chip">${x.tourTitle || x.title || 'تور'}</span>`).join('') || r.trailText || '—'}</td>
      ...
      .join('') || '<tr><td colspan="4">هنوز ردپایی ثبت نشده است.</td></tr>'
  }</tbody></table>`;
}

function galleryText(t) {
  return [t.img, ...(t.gallery || [])]
    .filter(Boolean)
    .filter((v, i, a) => a.indexOf(v) === i)
    .join('\n');
}
function tourGalleryList(t) {
  return [t.img, ...(t.gallery || [])].filter(Boolean).filter((v, i, a) => a.indexOf(v) === i);
}
function renderTourImages() {
  const box = $('adminTourImagesTable');
  if (!box) return;
  const list = tours();
  ...
  <div class="tour-image-manager">${list
    .map(t => {
      ...
      <div class="admin-gallery-preview booking-preview">${
        gallery
          .slice(0, 12)
          .map(
            (src, i) =>
              `<span class="booking-preview-item"><img src="${src}"><button onclick="removeTourGalleryImage(${t.id},${i})">×</button></span>`
          )
          .join('') || '<span class="small">هنوز عکسی ثبت نشده است.</span>'
      }</div>
    </div>`;
    })
    .join('')}</div>`;
...
function saveTourImageSettings(id) {
  ...
  const single = [0, 1, 2, 3, 4, 5, 6, 7]
    .map(n => $(`tourGalleryUrl_${id}_${n}`)?.value?.trim())
    .filter(Boolean);
  const bulk = ($(`tourGalleryUrls_${id}`)?.value || '')
    .split('\n')
    .map(x => x.trim())
    .filter(Boolean);
...
function uploadTourMainImage(id, files) {
  const file = [...(files || [])][0];
  if (!file) return;
  resizeImageFile(file, 1100, 0.76).then(url => {
    if (!url) return alert('عکس معتبر انتخاب نشد');
    const ok = saveTours(
      tours().map(t => {
...
function uploadTourGalleryImages(id, files) {
  const arr = [...(files || [])].slice(0, 12);
  if (!arr.length) return;
  Promise.all(arr.map(f => resizeImageFile(f, 1000, 0.74))).then(urls => {
    urls = urls.filter(Boolean);
    ...
    const ok = saveTours(
      tours().map(t => {
        ...
        return {
          ...t,
          img: t.img || gallery[0] || DEFAULT_IMG,
          gallery,
          lastEditedBy: 'مدیریت',
          lastEditedAt: new Date().toISOString(),
        };
...
function saveStaffAccount() {
  const name = $('staffFullName').value.trim(),
    username = $('staffUsername').value.trim(),
    password = $('staffPassword').value.trim(),
    taskNote = ($('staffTaskNote')?.value || '').trim(),
  ...
  const item = {
    id: i >= 0 ? list[i].id : 'staff-' + Date.now(),
    name: name || username,
    username,
    password,
    taskNote,
  ...
  ['staffFullName', 'staffUsername', 'staffPassword', 'staffTaskNote'].forEach(id => {
    if ($(id)) $(id).value = '';
  });
  $('staffActive').value = 'true';
  renderStaffAccounts();
  showToast('تیم فروش ذخیره شد');
}
function editStaffAccount(username) {
  const a = staffAccounts().find(x => x.username === username);
  if (!a) return;
  $('staffFullName').value = a.name || '';
  $('staffUsername').value = a.username;
  $('staffPassword').value = a.password;
  if ($('staffTaskNote')) $('staffTaskNote').value = a.taskNote || '';
  $('staffActive').value = String(a.active !== false);
}
function toggleStaffAccount(username) {
  const list = staffAccounts().map(a =>
    a.username === username ? { ...a, active: !(a.active !== false) } : a
  );
  saveStaffAccounts(list);
  renderStaffAccounts();
}
function deleteStaffAccount(username) {
  ...
  saveStaffAccounts(staffAccounts().filter(a => a.username !== username));
  renderStaffAccounts();
}
function renderStaffAccounts() {
  const list = staffAccounts(),
    box = $('staffAccountsTable');
  if (!box) return;
...
function editHotelItem(id) {
  const h = hotelCatalog().find(x => x.id === id);
  if (!h) return;
...
function saveHotelItem() {
  ...
  if (!name) {
    alert('نام لاتین هتل را وارد کنید');
    return;
  }
  ...
  const item = {
    ...
    photos: [
      $('hotelImageUrl')?.value?.trim() || '',
      ...String($('hotelPhotosText')?.value || '')
        .split(',')
        .map(x => x.trim()),
    ].filter(Boolean),
...
function addCurrentTourHotel(tourId) {
  const name = prompt('نام لاتین هتل جدید:');
  if (!name) return;
  const star = Number(prompt('ستاره هتل:', 3));
  if (!star) return;
  ...
  const ts = tours().map(t =>
    t.id === tourId
      ? {
          ...
          hotels: [
            ...(t.hotels || []),
            { hotelId: 'admin-' + Date.now(), name, star, price, capacity, showInBuyer: true },
          ],
        }
      : t
  );
  saveTours(ts);
  renderCurrentTourHotels();
  showToast('هتل اضافه شد');
}

function hotelPhotosText(h) {
  return (h.photos || h.images || []).filter(Boolean).join('\n');
}
function saveHotelAdminEdit(tourId, index) {
  const ts = tours().map(t => {
    ...
    hs[index] = {
      ...
      photos: $(`hotelPhotos_${tourId}_${index}`)
        .value.split('\n')
        .map(x => x.trim())
        .filter(Boolean),
...
function uploadHotelPhotos(tourId, index, files) {
  ...
  Promise.all(
    ...
    renderCurrentTourHotels();
    showToast('عکس‌های هتل ذخیره شد');
  });
}
...
function toggleCurrentTourHotel(tourId, index) {
  const ts = tours().map(t => {
    if (Number(t.id) !== Number(tourId)) return t;
    const hotels = (t.hotels || []).map((h, i) =>
      i === Number(index) ? { ...h, showInBuyer: !(h.showInBuyer !== false) } : h
    );
...
function removeCurrentTourHotel(tourId, index) {
  ...
  saveTours(
    tours().map(t =>
      t.id === tourId ? { ...t, hotels: (t.hotels || []).filter((_, i) => i !== index) } : t
    )
  );
  renderCurrentTourHotels();
}
...
function renderCurrentTourHotels() {
  ...
  const box = $('currentTourHotelsTable');
  if (!box) return;
  const t = findTour(tourId);
  if (!t) {
    box.innerHTML = '<div class="empty-state-mini">تور انتخاب نشده است.</div>';
  ...
  <div class="admin-hotel-list">${
    list
      .map(
        (h, i) => `<div class="admin-hotel-card">
...
function toggleTourHotel(tourId, index) {
  ...
  saveTours(ts);
  renderCurrentTourHotels();
  showToast('وضعیت نمایش هتل تغییر کرد');
}
...
function headerIndex(headers, key) {
  ...
  for (const a of aliases) {
    const i = headers.indexOf(normalizeHeader(a));
    if (i >= 0) return i;
  }
  return -1;
}
function cellByKey(row, headers, key) {
...
function importPriceSheet(inputId, resultId) {
  ...
  reader.onload = () => {
    ...
    if (rows.length < 2) {
      $(resultId).textContent = 'فایل خالی است.';
      return;
    }
    ...
    rows.slice(1).forEach(r => {
      const tourId = Number(cellByKey(r, headers, 'tour_id') || 0),
        hotelName = String(cellByKey(r, headers, 'hotel_name_latin') || '').trim(),
        star = Number(cellByKey(r, headers, 'hotel_star') || 0);
      const price = Number(cellByKey(r, headers, 'price') || 0),
        cap = Number(cellByKey(r, headers, 'capacity') || 0);
      const showRaw = String(cellByKey(r, headers, 'show_in_buyer') || 'TRUE').toLowerCase();
      const oldPrice = Number(cellByKey(r, headers, 'special_old_price') || 0),
        newPrice = Number(cellByKey(r, headers, 'special_new_price') || 0),
        pct = Number(cellByKey(r, headers, 'special_discount_percent') || 0);
      ...
      if (endFa) {
        t.dealEndsAtFa = endFa;
        const g = parseFaDiscountDate(endFa);
        if (g) t.dealEndsAt = g.toISOString();
      }
      const h = (t.hotels || []).find(
        x =>
          String(x.name).trim().toLowerCase() === hotelName.toLowerCase() && Number(x.star) === star
      );
      if (h) {
...
function jalaliToGregorian(jy, jm, jd) {
  ...
  let gm = 1,
    d = gd;
  for (; gm <= 12 && d > sal[gm]; gm++) d -= sal[gm];
  return { gy, gm, gd: d };
}
...
function hotelCatalogPhotos(h) {
  return [
    ...new Set(
      [h?.image || h?.img || '', ...(h?.photos || h?.images || []).filter(Boolean)].filter(Boolean)
    ),
  ].slice(0, 18);
}
...
function updateCatalogHotel(id, patch) {
  saveHotelCatalog(
    hotelCatalog().map(h => (h.id === id ? normalizeHotelCatalogItem({ ...h, ...patch }) : h))
  );
}
function addHotelCatalogPhotoUrl(id) {
  const input = $('hotelPhotoUrl_' + id);
  const url = input?.value?.trim();
  if (!url) return alert('URL عکس را وارد کن');
  const h = hotelCatalog().find(x => x.id === id);
  if (!h) return;
...
function uploadHotelCatalogPhotoFiles(id, files) {
  ...
  if (!arr.length) return;
  Promise.all(arr.map(f => resizeImageFile(f, 900, 0.72))).then(urls => {
    const h = hotelCatalog().find(x => x.id === id);
    if (!h) return;
...
function hotelDestinations() {
  return [...new Set(hotelCatalog().map(h => h.destination || h.dest || 'عمومی'))].sort((a, b) =>
    String(a).localeCompare(String(b), 'fa')
  );
}
function renderHotelCatalog() {
  const box = $('hotelCatalogTable');
  if (!box) return;
  ...
  list.sort(
    (a, b) =>
      String(a.destination || '').localeCompare(String(b.destination || ''), 'fa') ||
      Number(a.star) - Number(b.star) ||
      String(a.nameLatin || '').localeCompare(String(b.nameLatin || ''))
  );
  const grouped = list.reduce((acc, h) => {
    const d = h.destination || h.dest || 'عمومی';
    (acc[d] || (acc[d] = [])).push(h);
    return acc;
  }, {});
  ...
  box.innerHTML = `<div class="hotel-catalog-summary hotel-catalog-summary-upgraded">
    ...
    Object.entries(grouped)
      ...
      ${rows
        .map(
          h => `<div class="admin-hotel-card admin-hotel-card-with-photo hotel-bank-card ${h.enabledForStaff !== false ? 'active-for-staff' : 'off-for-staff'}">
...
function toggleHotelDestination(dest, on) {
  saveHotelCatalog(
    hotelCatalog().map(h =>
      (h.destination || h.dest || 'عمومی') === dest ? { ...h, enabledForStaff: !!on } : h
    )
  );
  renderHotelCatalog();
...
function bulkToggleHotelCatalog(on) {
  if (!confirm(on ? 'همه هتل‌ها برای فروش نمایش داده شوند؟' : 'همه هتل‌ها از پنل فروش مخفی شوند؟'))
    return;
...
function toggleVisaItem(id) {
  saveVisaServices(
    visaServices().map(v => (v.id === id ? { ...v, active: v.active === false } : v))
  );
  renderVisas();
}
function deleteVisaItem(id) {
  if (confirm('حذف شود؟')) {
    saveVisaServices(visaServices().filter(v => v.id !== id));
    renderVisas();
  }
}
...
function airlineLogoAdmin(a) {
  return a.logo
    ...
    : `<span>${(a.nameFa || a.nameEn || 'A').slice(0, 1)}</span>`;
}
function renderAirlineCatalog() {
  const box = $('airlineCatalogTable');
  if (!box) return;
  ...
  if (q)
    list = list.filter(a => (a.nameFa + ' ' + a.nameEn + ' ' + a.code).toLowerCase().includes(q));
  ...
  const grouped = list.reduce((acc, a) => {
    const k = a.type === 'domestic' ? 'داخلی' : 'خارجی';
    (acc[k] || (acc[k] = [])).push(a);
    return acc;
  }, {});
  box.innerHTML = `<div class="airline-bank-summary">
    ...
    Object.entries(grouped)
      .map(
        ([group, rows]) => `<div class="airline-group-card">
    <div class="airline-group-head"><h4>${group}</h4><span>${faNum(rows.length)} مورد</span></div>
    <div class="airline-card-list">${rows
      .map(
        a => `<div class="airline-admin-card ${a.enabledForStaff !== false ? 'active-for-staff' : 'off-for-staff'}">
      ...
      .join('') || '<div class="empty-state-mini">ایرلاینی پیدا نشد.</div>'
  }`;
}
function saveAirlineItem() {
  ...
  ['airlineNameFa', 'airlineNameEn', 'airlineCode', 'airlineLogo', 'airlineNote'].forEach(id => {
    if ($(id)) $(id).value = '';
  });
...
function editAirlineItem(id) {
  const a = airlineCatalog().find(x => x.id === id);
  if (!a) return;
...
function toggleAirlineItem(id) {
  saveAirlineCatalog(
    airlineCatalog().map(a =>
      a.id === id ? { ...a, enabledForStaff: !(a.enabledForStaff !== false) } : a
    )
  );
  renderAirlineCatalog();
...
function bulkToggleAirlines(on) {
  if (
    !confirm(
      on ? 'همه ایرلاین‌ها برای فروش نمایش داده شوند؟' : 'همه ایرلاین‌ها از پنل فروش مخفی شوند؟'
    )
  )
    return;
...
function renderAllHotelNames(list = hotelCatalog()) {
  ...
  box.innerHTML =
    rows
      .map(
        h =>
          `<span><b dir="ltr">${h.nameLatin || '—'}</b><small>${h.destination || h.dest || 'عمومی'} | ${h.star}★ ${h.meal ? `| ${h.meal}` : ''}</small></span>`
      )
      .join('') || '<em>هتلی ثبت نشده است.</em>';
}
function renderAllVisaNames(list = visaServices()) {
  const box = $('allVisaNamesList');
  if (!box) return;
  box.innerHTML =
    list
      .map(
        v =>
          `<span><b>${v.country || '—'} ${v.city ? `- ${v.city}` : ''}</b><small>${v.type || 'توریستی'} | ${v.active !== false ? 'فعال' : 'غیرفعال'}</small></span>`
      )
      .join('') || '<em>ویزایی ثبت نشده است.</em>';
}
function currencyAdminCard(c) {
  return `<div class="currency-admin-card ${c.enabledForBuyer ? 'active-for-buyer' : 'off-for-buyer'}">
...
function renderCurrencyCatalog() {
  const box = $('currencyCatalogTable');
  if (!box) return;
  ...
  if (q)
    list = list.filter(c =>
      (c.code + ' ' + c.nameFa + ' ' + c.nameEn + ' ' + c.symbol).toLowerCase().includes(q)
    );
...
function saveCurrencyItem() {
  const code = String($('currencyCode')?.value || '')
    .trim()
    .toUpperCase();
  if (!code) return alert('کد ارز را وارد کن');
  const item = {
    id: 'cur-' + code,
    code,
    nameFa: $('currencyNameFa')?.value?.trim() || currencyNameFa(code),
    nameEn: $('currencyNameEn')?.value?.trim() || currencyNameEn(code),
    symbol: $('currencySymbol')?.value?.trim() || code,
    rate: $('currencyRate')?.value?.trim() || '',
    enabledForBuyer: $('currencyBuyerShow')?.value === 'true',
  };
  const list = currencyCatalog();
  const i = list.findIndex(c => c.code === code);
  if (i >= 0) list[i] = { ...list[i], ...item };
  else list.push(item);
  saveCurrencyCatalog(list);
  ['currencyCode', 'currencyNameFa', 'currencyNameEn', 'currencySymbol', 'currencyRate'].forEach(
    id => {
      if ($(id)) $(id).value = '';
    }
  );
  if ($('currencyBuyerShow')) $('currencyBuyerShow').value = 'false';
  renderCurrencyCatalog();
  showToast('ارز ذخیره شد');
}
function editCurrencyItem(code) {
  const c = currencyCatalog().find(x => x.code === code);
  if (!c) return;
  $('currencyCode').value = c.code;
  $('currencyNameFa').value = c.nameFa || '';
  $('currencyNameEn').value = c.nameEn || '';
  $('currencySymbol').value = c.symbol || '';
  $('currencyRate').value = c.rate || '';
  $('currencyBuyerShow').value = String(c.enabledForBuyer === true);
  document.getElementById('admin-currencies')?.scrollIntoView({ behavior: 'smooth' });
}
function toggleCurrencyItem(code) {
  saveCurrencyCatalog(
    currencyCatalog().map(c =>
      c.code === code ? { ...c, enabledForBuyer: !c.enabledForBuyer } : c
    )
  );
  renderCurrencyCatalog();
  showToast('وضعیت نمایش ارز تغییر کرد');
}
function deleteCurrencyItem(code) {
  if (!confirm('این ارز حذف شود؟')) return;
  saveCurrencyCatalog(currencyCatalog().filter(c => c.code !== code));
  renderCurrencyCatalog();
}
function bulkToggleCurrencies(on) {
  if (
    !confirm(on ? 'همه ارزها در پنل خریدار نمایش داده شوند؟' : 'همه ارزها از پنل خریدار مخفی شوند؟')
  )
    return;
  saveCurrencyCatalog(currencyCatalog().map(c => ({ ...c, enabledForBuyer: !!on })));
  renderCurrencyCatalog();
}
function resetCurrencies() {
  if (!confirm('لیست ارزها به حالت جهانی اولیه برگردد؟')) return;
  saveCurrencyCatalog(defaultCurrencyCatalog());
  renderCurrencyCatalog();
}
...
function renderVisas() {
  ...
  <div class="admin-edit-list">${
    list
      .map(
        v => `<div class="admin-edit-row visa-admin-row">
    <div>
    ...
    <div class="admin-edit-row-actions">
      ...
      .join('') || '<div class="empty-state-mini">هنوز ویزایی ثبت نشده است.</div>'
  }</div>`;
}
function saveVisaItem() {
  ...
  if (i >= 0) list[i] = { ...list[i], ...item, active: list[i].active !== false };
  else list.push(item);
...
function editVisaItem(id) {
  const v = visaServices().find(x => x.id === id);
  if (!v) return;
...
function addDiscount() {
  const ds = discounts();
  ds.push({
    code: $('dcode').value.toUpperCase(),
    type: $('dtype').value,
    value: Number($('dvalue').value),
    min: Number($('dmin').value) || 0,
    tourId: $('dtour').value,
    active: true,
    used: 0,
    limit: 100,
...
function renderOrders() {
  ...
  if (q)
    os = os.filter(
      o => o.name.toLowerCase().includes(q) || o.phone.includes(q) || o.id.toLowerCase().includes(q)
    );
  if (sf !== 'all') os = os.filter(o => o.status === sf);
...
function changeStatus(id, v) {
  saveOrders(orders().map(o => (o.id === id ? { ...o, status: v } : o)));
  renderOrders();
  renderCustomerTrailAdmin();
}
function changeNote(id, v) {
  saveOrders(orders().map(o => (o.id === id ? { ...o, adminNote: v } : o)));
}
...
function resetDefaultHotels() {
  if (!confirm('بانک هتل‌ها به لیست واردشده اولیه برگردد؟ وضعیت‌های فعال/غیرفعال قبلی پاک می‌شود.'))
    return;
  saveHotelCatalog(defaultHotelCatalog());
  const sel = $('hotelCatalogDestFilter');
...
function syncCatalogHotelsToTours() {
  ...
  if (!active.length) {
    alert('هیچ هتلی برای فروش فعال نیست');
    return;
  }
  ...
  const ts = tours().map(t => {
    ...
    const existing = new Set(
      (t.hotels || []).map(h => String(h.name || h.nameLatin || '').toLowerCase())
    );
    const add = matches
      .filter(h => !existing.has(String(h.nameLatin || '').toLowerCase()))
      .map(h => ({
        hotelId: h.id,
        name: h.nameLatin,
        star: h.star,
