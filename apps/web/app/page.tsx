    ...
    <>
      ...
      <div className="relative z-[2]">
        <section id="experiences" className="section-grid relative overflow-hidden border-t border-white/5 bg-[#06080d] px-4 py-28">
          <div className="mx-auto max-w-6xl">
            <Reveal className="mx-auto max-w-2xl text-center">
              <p className="text-xs uppercase tracking-[.34em] text-amber-200/80">Curated around you</p>
              <h2 className="mt-4 text-balance text-4xl font-semibold tracking-[-.045em] sm:text-6xl">Paris, with the noise removed.</h2>
              <p className="mx-auto mt-5 max-w-xl text-sm leading-7 text-white/70">A premium travel experience where planning, access and local taste come together in one calm flow.</p>
            </Reveal>

            <div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {experiences.map((item, index) => (
                <Reveal key={item.title} delay={index * .08}>
                  <article className="glass group min-h-72 rounded-[2rem] p-6 transition duration-300 hover:-translate-y-2 hover:border-amber-200/25">
        ...
        <section className="relative overflow-hidden bg-black px-4 py-32">
          <div className="pointer-events-none absolute left-1/2 top-0 h-72 w-[70vw] -translate-x-1/2 rounded-full bg-amber-400/10 blur-[110px]" />
          <div className="mx-auto max-w-6xl">
            <Reveal className="mx-auto max-w-3xl text-center">
              <p className="font-serif text-4xl italic leading-tight text-white/88 sm:text-6xl">Built for travelers who value time, taste and detail.</p>
              <div className="mt-8 flex flex-wrap justify-center gap-x-10 gap-y-4 text-xs font-semibold tracking-[.2em] text-white/55">
                <span>PRIVATE PLANNING</span><span>LOCAL TASTE</span><span>DINING</span><span>TRANSFERS</span><span>ON-TRIP SUPPORT</span>
              </div>
            </Reveal>
          </div>
        </section>
          ...
          <div className="mx-auto grid max-w-6xl gap-14 lg:grid-cols-[.8fr_1.2fr]">
            <Reveal>
              <div className="sticky top-28">
                <p className="text-xs uppercase tracking-[.32em] text-amber-200/80">How we work</p>
                <h2 className="mt-4 text-4xl font-semibold tracking-[-.045em] sm:text-5xl">A sharp plan. A soft landing.</h2>
                <p className="mt-5 max-w-md text-sm leading-7 text-white/70">The experience is designed to feel spontaneous while the difficult parts are already handled behind the scenes.</p>
              ...
              {principles.map(([no, title, text], index) => (
                <Reveal key={no} delay={index * .07}>
                  <article className="glass min-h-72 rounded-[2rem] p-6">
                    <div className="flex items-center justify-between text-xs text-white/60"><span>{no}</span><Sparkles className="size-4" /></div>
        ...
        <section id="stories" className="relative overflow-hidden bg-[#071020] px-4 py-28">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_40%,rgba(212,175,55,.14),transparent_25%),radial-gradient(circle_at_80%_70%,rgba(115,183,255,.12),transparent_28%)]" />
          <div className="relative mx-auto max-w-6xl">
            <Reveal className="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end">
              <div>
                <p className="text-xs uppercase tracking-[.32em] text-amber-200/80">Traveler story</p>
            ...
            <Reveal>
              <div className="glass grid overflow-hidden rounded-[2rem] lg:grid-cols-[.75fr_1.25fr]">
                ...
                <div className="p-7 sm:p-10 lg:p-12">
                  <BadgeCheck className="size-8 text-amber-200" />
                  ...
                  <div className="mt-10 grid grid-cols-3 gap-3 border-t border-white/10 pt-7">
                    <div><div className="text-3xl font-semibold">4</div><div className="mt-1 text-xs text-white/65">Days in Paris</div></div>
                    <div><div className="text-3xl font-semibold">11</div><div className="mt-1 text-xs text-white/65">Reservations</div></div>
                    <div><div className="text-3xl font-semibold">0</div><div className="mt-1 text-xs text-white/65">Planning stress</div></div>
                  </div>
                </div>
              </div>
            </Reveal>
        ...
        <section className="bg-black px-4 py-28">
          <div className="mx-auto max-w-6xl">
            <Reveal className="grid gap-6 lg:grid-cols-3">
              <div className="glass rounded-[2rem] p-7"><Gem className="size-6 text-amber-200" /><div className="mt-16 text-4xl font-semibold">24/7</div><p className="mt-2 text-sm text-white/68">Concierge support during your trip.</p></div>
              <div className="glass rounded-[2rem] p-7"><HeartHandshake className="size-6 text-amber-200" /><div className="mt-16 text-4xl font-semibold">1:1</div><p className="mt-2 text-sm text-white/68">A personal plan, not a generic package.</p></div>
              <div className="glass rounded-[2rem] p-7"><Sparkles className="size-6 text-amber-200" /><div className="mt-16 text-4xl font-semibold">48h</div><p className="mt-2 text-sm text-white/68">Typical turnaround for a first itinerary.</p></div>
            </Reveal>
          </div>
        </section>
        ...
        <section id="contact" className="section-grid bg-[#07090f] px-4 py-28">
          <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-start">
            <Reveal>
              <p className="text-xs uppercase tracking-[.32em] text-amber-200/80">Start a conversation</p>
              ...
              <p className="mt-6 max-w-md text-sm leading-7 text-white/70">Share the basics and we will shape a first direction around your dates, pace, interests and non-negotiables.</p>
            </Reveal>
        ...
        <footer className="border-t border-white/5 bg-black px-4 py-10">
          <div className="mx-auto flex max-w-6xl flex-col justify-between gap-5 text-xs text-white/60 sm:flex-row">
            <div className="font-semibold tracking-[.22em] text-white/70">AURA PARIS</div>
            <div>Private journeys • Paris, France</div>
            <div>© 2026</div>
          </div>
        </footer>
      </div>
